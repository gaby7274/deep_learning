...
class Encoder(nn.Module):
    ...
    def forward(self, input_sequence, padding_mask=None):
        ...
        if self.pos_embedding is not None:
            ...
            sequence_idx = torch.arange(seq_length, device=input_sequence.device)
            pos_emb = (
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        batch_size, l, h = input_embeddings.shape

        ## ADD positional information to embeddings.
        if self.pos_embedding is not None:
            sequence_idx = torch.arange(l, device=input_sequence.device)
            pos_emb = (
                self.pos_embedding(sequence_idx)
                .reshape(1, l, h)
                .expand(batch_size, l, h)
            )
